...
class PathEvaluator {
     ...
     */
    static String chooseBestPathDelta (final List<String> paths, final String reference) {
        if (paths.size () == 0) {
            return "";
        }
        ...
        int min = WordEvaluator.valueOfPath (PathEvaluator.remainingPath (paths.get (0), reference));
        String valMin = paths.get (0);
        for (int i = 1 ; i < paths.size () ; i++) {
            final int newVal = WordEvaluator.valueOfPath (PathEvaluator.remainingPath (paths.get (i), reference));
            if (newVal < min) {
                valMin = paths.get (i);
                min = newVal;
            }
    ...
    /**
     * When a path is choosed, this method is used to find the remaining cost in
     * a math expr.
     *
     * @param path1
    ...
    static String remainingPath (final String path1, final String path2) {
        ...
        matcher.find ();
        matcherRef.find ();
        final String negativePath1 = matcher.group (2) == null ? "" : matcher.group (2);
        final String positivePath1 = matcher.group (5) == null ? "" : matcher.group (5);
        final String negativePath2 = matcherRef.group (2) == null ? "" : matcherRef.group (2);
        final String positivePath2 = matcherRef.group (5) == null ? "" : matcherRef.group (5);
