...
public class WordEvaluator {
  ...
  private static int evaluate (final String letters) {
    ...
    for (int i = 0 ; i < letters.length () ; i++) {
      if ((letters.charAt (i) == 'a') || (letters.charAt (i) == 'e')
          ...
          || (letters.charAt (i) == 'u') || (letters.charAt (i) == 'y')) {
        val++;
      } else if (Character.isLowerCase (letters.charAt (i))
          || Character.isDigit (letters.charAt (i))) {
        val += 5;
      }
    }
    return val;
  ...
  public static int valueOfNotTranslated (final String equation) {
    ...
    remainders = remainders.replaceAll ("\\+", "").replaceAll ("\\-", "")
