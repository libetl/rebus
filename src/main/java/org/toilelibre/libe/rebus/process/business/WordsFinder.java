...
/**
 * Contains a method to add words and a method to find words in our
 * WordsFindStruct
 * 
 * @author LiBe
...
public class WordsFinder {
   ...
   * Needs a path and two words.
   * This equation must be true : w1 - w2 = path
   * @param wfs the struct
   * @param pathNegative - [X] +  Y  the value in brackets
   * @param pathPositive -  X  + [Y] the value in brackets
  ...
  private static void addOnePath (final FSM<Set<Pair<Word, Word>>, String> wfs,
    ...
    FSM<Set<Pair<Word, Word>>, String> fsm = wfs;
    //We follow the correct path
    fsm = WordsFinder.goRecursive (fsm, "-", pathNegative);
    fsm = WordsFinder.goRecursive (fsm, "+", pathPositive);
    
    //Then we add the pair
    if (fsm.getData () == null) {
      fsm.setData (new HashSet<Pair<Word, Word>> ());
    }
    fsm.getData ().add (pair);
  }

  /**
   * Adds a pair in the struct
   * After this operation, we will be able to replace
   * the value of WordsDifference.difference (w1, w2) by
   * :w1 - :w2
   * 
   * This method is called when the stuct is under
   * construction.
   * 
   * @param wfs the struct
   * @param w1 first word of the pair
  ...
  public static void addPath (final FSM<Set<Pair<Word, Word>>, String> wfs, final Word w1,
    ...
    final String difference2to1 = WordsDifference.difference (w2, w1);
    ...
    WordsFinder.addOnePath (wfs, negative, positive, w1, w2);
    negative = matcher2.group (2);
    positive = matcher2.group (5);
    WordsFinder.addOnePath (wfs, negative, positive, w2, w1);
  }

  /**
   * Public method to find a good pair (substraction) of words to 
   * replace a path 
   * @param wfs the struct 
   * @param path the path to explore in our struct
   * @param wordToAvoid we can specify a word we don't want to see
   *                    to avoid things like :rhino + (:hornet - :rhino)
   * @param numLettersMissing if the path is incomplete, how many letters
   *                          can we add in it ?
   * @param lengthDelta how much longer or shorter can be the path ?
  ...
  public static String[] findNearestPair (final FSM<Set<Pair<Word, Word>>, String> wfs,
    ...
    if ("".equals (path)) { return new String[] { "", "" }; }
    
    //Now, we retrieve each interesting path according to the two
    //integers specified (numLettersMissing and lengthDelta)
    final Map<String, Set<Pair<Word, Word>>> map = WordsFinder
        .searchPairsByPath (wfs, path, wordToAvoid, numLettersMissing,
            lengthDelta);
    
    //We choose the best matching path to our path parameter
    final String bestPath = PathEvaluator.chooseBestPathDelta (
        new LinkedList<String> (map.keySet ()), path);
    ...
    if (bestPath.length () == 0) { return new String[] { path, "" }; }

    //We should pick one pair randomly in the set
    @SuppressWarnings ("unchecked")
    final Pair<Word, Word> pair = (Pair<Word, Word>) bestSet.toArray ()[new Random ()
        .nextInt (bestSet.size ())];
    ...
    return new String[] {
        "+ (" + pair.getObj1 () + " - " + pair.getObj2 () + " " + remaining
            + ")", remaining };
  }
   ...
   * 
   * To sum up, this method looks deeply into the 
   * FSM-Struct to look for every set of pairs that could
   * best fit our submitted letters (in the parameters
   * negative and positive).
   * It avoids adding the pairs containing the wordToAvois
   * 
   * It can follow the paths with a [numLettersMissing]
   * tolerance if no transition is found.
   * 
   * It takes every set of pairs when the currentPath is
   * around the pathLength (with a tolerance of 
   * lengthDelta)
   * 
   * currentPathNegative and currentPathPositive are values
   * used by recursion. If called from outside, these values
   * are ""
   * 
   * @param fsm the FSM we are in. Can be a child of the
   *        WordsFindStruct, or the WordsFindStruct itself
   * @param wordToAvoid the word to avoid
   * @param negative remaining negative path to explore
   * @param positive remaining positive path to explore
   * @param numLettersMissing tolerance to incomplete paths
   * @param lengthDelta tolerance to paths lengths
   * @param pathLength current path length. Also is the degree
   *                   of recursion
   * @param currentPathNegative recursion variable. 
   *                            Contains the current negative path
   * @param currentPathPositive recursion variable. 
   *                            Contains the current positive path
   * @return all the sets of pairs arranged by paths
   */
  private static Map<String, Set<Pair<Word, Word>>> findNearestPathsData (
      final FSM<Set<Pair<Word, Word>>, String> fsm, final Word wordToAvoid,
      final String negative, final String positive,
      final int numLettersMissing, final int lengthDelta,
      final int pathLength, final String currentPathNegative,
    ...
    final int currentLength = currentPathNegative.length ()
        + currentPathPositive.length ();
    final String currentPath = ToStringExpr.buildPathText (currentPathNegative,
        currentPathPositive);
    ...
    if (fsm.getTransitions () != null) {
      for (final String s : fsm.getTransitions ().keySet ()) {
        ...
        if (s.startsWith ("+") && (positive.indexOf (s.charAt (1)) != -1)) {
          ...
          String newPositive = positive.replaceFirst ("" + s.charAt (1), "");
          result.putAll (WordsFinder.findNearestPathsData (fsm.getTransitions ()
              .get (s), wordToAvoid, negative, newPositive,
              numLettersMissing, lengthDelta, pathLength,
              currentPathNegative, currentPathPositive + s.charAt (1)));
        } else if (s.startsWith ("-")
            && (negative.indexOf (s.charAt (1)) != -1)) {
          //We found a matching negative transition in the path
          String newNegative = negative.replaceFirst ("" + s.charAt (1), "");
          result.putAll (WordsFinder.findNearestPathsData (fsm.getTransitions ()
              .get (s), wordToAvoid, newNegative, positive,
              numLettersMissing, lengthDelta, pathLength,
              currentPathNegative + s.charAt (1), currentPathPositive));
        } else if (s.startsWith ("-") && (numLettersMissing > 0)) {
          //We can use the tolerance as a joker for a negative transition
          result.putAll (WordsFinder.findNearestPathsData (fsm.getTransitions ()
              .get (s), wordToAvoid, negative, positive,
              numLettersMissing - 1, lengthDelta, pathLength,
              currentPathNegative + s.charAt (1), currentPathPositive));
        } else if (s.startsWith ("+") && (numLettersMissing > 0)) {
   ...
   * @param fsm FSM to explore
   * @param symbol "+" or "-"
   * @param path path to explore
   * @return the children FSM, even empty
   */
  private static FSM<Set<Pair<Word, Word>>, String> goRecursive (
      FSM<Set<Pair<Word, Word>>, String> fsm, final String symbol,
      final String path) {
    if (path == null) { return fsm; }
    for (int i = 0 ; i < path.length () ; i++) {
      ...
      if (fsm.getTransitions ().get (symbol + path.charAt (i)) == null) {
        fsm.getTransitions ().put (symbol + path.charAt (i),
            new FSM<Set<Pair<Word, Word>>, String> ());
      }
   ...
   * @param wordToAvoid word to avoid
   * @param set the set containing the pairs
   */
  private static void removePairsWithWordToAvoid (final Word wordToAvoid,
      final Set<Pair<Word, Word>> set) {
    final List<Pair<Word, Word>> toRemove = new LinkedList<Pair<Word, Word>> ();
    for (final Pair<Word, Word> pair : set) {
      if (pair.getObj1 ().equals (wordToAvoid)
          || pair.getObj2 ().equals (wordToAvoid)) {
        toRemove.add (pair);
      }
    }
   ...
   * @param wfs our struct
   * @param path the path in mathematical expression
   * @param wordToAvoid a word to avoid
   * @param numTransitionsMissing tolerance for paths
   * @param lengthDelta tolerance for path length
   * @return all the sets of pairs arranged by paths
   */
  private static Map<String, Set<Pair<Word, Word>>> searchPairsByPath (
    ...
    final String negative = (matcher1.group (2) == null ? "" : matcher1
        .group (2));
    final String positive = (matcher1.group (5) == null ? "" : matcher1
        .group (5));
    
    //We proceed to the call of the recursion method here
    final FSM<Set<Pair<Word, Word>>, String> fsm = wfs;
    return WordsFinder.findNearestPathsData (fsm, wordToAvoid, negative,
        positive, numTransitionsMissing, lengthDelta, negative.length ()
            + positive.length (), "", "");
  }
}
