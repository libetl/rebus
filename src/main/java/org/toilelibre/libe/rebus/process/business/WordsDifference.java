...
public class WordsDifference {
  ...
  public static String difference (final Word word, final Word otherWord) {
    ...
    // Reading both words
    while ((i < wordSortedLetters.length ())
        && (j < otherWordSortedLetters.length ())) {
      ...
      if (wordSortedLetters.charAt (i) < otherWordSortedLetters.charAt (j)) {
        ...
        i++;
      } else if (wordSortedLetters.charAt (i) > otherWordSortedLetters
          .charAt (j)) {
    ...
    return (remBuff.length () > 0 ? "- " + remBuff.toString () : "")
        + ((addBuff.length () > 0) && (remBuff.length () > 0) ? " + "
            + addBuff.toString () : "")
        + ((addBuff.length () > 0) && (remBuff.length () == 0) ? "+ "
            + addBuff.toString () : "");
  }
